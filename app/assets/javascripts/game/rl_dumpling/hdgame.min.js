	...
	(function() {
		...
		var q = {
			...
			payWithCredit:function(s){
				...
				$.ajax({
					type: "post",
					url: r.payUrl + "&start=" + q.start + "&limit=" + q.limit + "&playerId=" + g_config.playerId,
					error: function() {
						s.error && s.error();
						q.isLoad = false
					},
					success: function(u) {
						b.tlog("payWithCredit", u);
						var v = $.parseJSON(u);
						WechatMore.is_paid = v.isSuc
						s.success && s.success(v);
					}
				});
			}
		...
		function g(t, s) {
			...
			$(".poupClose,.poupClose2").off("click");
			setTimeout(function() {
				$(".poupClose,.poupClose2").on("click", function(x) {
					if (_manage) {
						return
					}
					x.preventDefault();
		...
		var o = function(s) {
			...
			if (_manage) {
				...
				u.push("&playerId=", g_config.playerId);
				if (g_config.isDoubleGame) {
				...
				if ((v - m) > 100) {
					...
					if (u > p) {
						...
						h.giftInit(function(w) {
							$("#resule-status-lotsBox .cartoonBox").addClass('move');
							hg.sound.play("yiy");
							setTimeout(function() {
								j.showGift(w)
								$("#resule-status-lotsBox .cartoonBox").removeClass('move');
							}, 1500)
						})
					}
					o = l;
		...
		j.init = function(k) {
			...
			if ((h.drawType === 0 || h.drawType === 2) && g_config.style != 61 && g_config.style != 27) {
				...
				if (h.drawType === 1 || g_config.style == 61 || g_config.style == 27) {
					...
					hg.sound.load(WechatMore.assets.audio_yaoyiyao_url, "yiy");
					window.addEventListener("devicemotion", g, false);
		...
		j.showResult = function(n) {
			...
			if (l.gameType == 3) {
				...
				if (l.gameScore == "fail") {
					...
					$(".resule-status-userImg").css("border-color", "#C72546");
				...
				if (l.isSuc) {
					...
					$("#resule-status-ribbon").removeClass("resule-status-ribbon").addClass("resule-status-faiRibbon");
					$(".resule-status-userImg").css("border-color", "#CACACA");
...
console.log( w );
	...
	b.initSound = function(g, h, f) {
		...
		j.load( WechatMore.assets.audio_button_url, "startButton");
		if (g) {
			$.each(g, function(l, m) {
				var o = m.path;
				if (l === 0) {
			...
			if (!g_config.isFromZhuliShare && isPublish) {
				wx.getNetworkType({
					success: function(r) {
						...
						$.ajax({
							url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=setBehavior&gameId=" + g_config.gameId + "&openId=" + g_config.openId + "&playerId=" + g_config.playerId,
							type: "post",
							data: {
								netType: t,
								sysType: q
	...
	b.initJsFoot = function(e) {
		...
		$(function() {
			...
			hg.on("home", $.throttle(function() {
				$.ajax({
					type: "post",
					url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=getJoinNum&gameId=" + g_config.gameId + "&openId=" + g_config.openId + "&playerId=" + g_config.playerId,
					success: function(k) {
						b.tlog("getJoinNum", k);
						var j = $.parseJSON(k);
						if (!isNaN(j.joinNum)) {
	...
	b.createQrImg = function(l, g) {
		...
		try {
			...
			h.src = WechatMore.assets.image_caewm_red_url;

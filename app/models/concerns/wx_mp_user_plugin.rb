module Concerns::WxMpUserPlugin
  ...
  def refresh_access_token!
    ...
    url = "https://api.weixin.qq.com/cgi-bin/component/api_authorizer_token?component_access_token=#{WechatOpen.component_access_token}"
    post_body = {
      component_appid:          Settings.wx_plugin.component_appid,
  ...
  # }
  def fetch_auth_info
    url = "https://api.weixin.qq.com/cgi-bin/component/api_get_authorizer_info?component_access_token=#{WechatOpen.component_access_token}"
    post_body = {component_appid: WechatOpen.component_appid, authorizer_appid: app_id}.to_json
    ...
    set_service_type_by_wx_auth(service_type_info: authorizer_info['service_type_info']['id'], verify_type_info: authorizer_info['verify_type_info']['id'])
    save!
  end

  def set_service_type_by_wx_auth(service_type_info: nil, verify_type_info: nil)
    verified = [0, 1, 2].include?(verify_type_info)
    self.service_type = if [0, 1].include?(service_type_info) # 订阅号
      verified ? WxMpUser.service_types[:auth_subscribe]  : WxMpUser.service_types[:subscribe]
    elsif service_type_info == 2 # 服务号
      verified ?  WxMpUser.service_types[:auth_service] :WxMpUser.service_types[:service]
    end
    self.service_type ||=  WxMpUser.service_types[:unknown]
  end

  def get_authorizer_option(option_name)
    url = "https://api.weixin.qq.com/cgi-bin/component/api_get_authorizer_option?component_access_token=#{WechatOpen.component_access_token}"
    post_body = {component_appid: Settings.wx_plugin.component_appid, authorizer_appid: app_id, option_name: option_name}.to_json
    result = HTTParty.post(url, body: post_body).to_h
  end

  def set_authorizer_option(option_name, option_value)
    url = "https://api.weixin.qq.com/cgi-bin/component/api_set_authorizer_option?component_access_token=#{WechatOpen.component_access_token}"
    post_body = {
      component_appid: Settings.wx_plugin.component_appid,
      authorizer_appid: app_id,
  ...
  def wx_user_oauth_url
    ...
    "https://open.weixin.qq.com/connect/oauth2/authorize?appid=#{app_id}&redirect_uri=#{wx_user_plugin_oauth_url}&response_type=code&scope=snsapi_base&state=STATE&component_appid=#{Settings.wx_plugin.component_appid}#wechat_redirect"
  end

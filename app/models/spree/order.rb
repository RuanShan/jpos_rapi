...
module Spree
  class Order < Spree::Base
    ...
    self.whitelisted_ransackable_attributes = %w[store_id completed_at email number state payment_state group_state shipment_state total order_type created_at updated_at user_id odd_card_paid odd_store_id payment_at]
        ...
        if order_type_card?
          ...
          self.sale_day.deposit_total = deposit_total
            ...
            if self.payment_at.present?
              payment_sale_day = SaleDay.on_date( self.payment_at ).where( user_id: self.created_by_id, store_id: self.store_id).first
              service_total =  Spree::Order. where( ["payment_at>? and payment_at<?",  self.payment_at.beginning_of_day, self.payment_at.end_of_day] ).where( created_by_id: self.created_by_id, store_id: self.store_id, order_type: [:normal] ).with_state(:cart).sum(:payment_total)
              ...
              # 找到 payment_at 对应的sale_day, 更新
              payment_sale_day.service_total = service_total
              payment_sale_day.service_posable_total = service_posable_total
              payment_sale_day.save!
            end
        end
        self.sale_day.save!
      end

module Spree
  module Admin
    class SellingPrepaidCardsController < ProductsController
      ...
      def update
        if params[:selling_prepaid_card][:taxon_ids].present?
          params[:selling_prepaid_card][:taxon_ids] = params[:selling_prepaid_card][:taxon_ids].split(',')
        end
        if params[:selling_prepaid_card][:option_type_ids].present?
          params[:selling_prepaid_card][:option_type_ids] = params[:selling_prepaid_card][:option_type_ids].split(',')
        end
        ...
        if @object.update_attributes(permitted_resource_params)
          ...
          @object.slug = @object.slug_was if @object.slug.blank?
      ...
      def destroy
        @object = model_class.friendly.find(params[:id])

        begin
          # TODO: why is @object.destroy raising ActiveRecord::RecordNotDestroyed instead of failing with false result
          if @object.destroy
            flash[:success] = Spree.t('notice_messages.product_deleted')
          else
            flash[:error] = Spree.t('notice_messages.product_not_deleted', error: @object.errors.full_messages.to_sentence)
          end
      ...
      def stock
        @variants = @object.variants.includes(*variant_stock_includes)
        @variants = [@object.master] if @variants.empty?
      ...
      def create_before
        return if params[:selling_prepaid_card][:prototype_id].blank?
        @prototype = Spree::Prototype.find(params[:selling_prepaid_card][:prototype_id])
      end
